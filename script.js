...
function parseDartsCSV(text) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        // NAME UMDREHEN VON "Vorname Nachname" ZU "Nachname, Vorname"
        let roherName = rowData[0];
        let anzeigeName = roherName;
        
        if (roherName.includes(' ') && !roherName.includes(',')) {
            const namensTeile = roherName.split(' ');
            if (namensTeile.length === 2) {
                // Bei "Vorname Nachname" -> "Nachname, Vorname"
                anzeigeName = `${namensTeile[1]}, ${namensTeile[0]}`;
            } else if (namensTeile.length > 2) {
                // Falls jemand zwei Vornamen hat (z. B. "Hans Peter Müller" -> "Müller, Hans Peter")
                const nachname = namensTeile.pop();
                const vorname = namensTeile.join(' ');
...
function generiereKarten(spielerDaten) {
    ...
    if (spielerDaten.length === 0) {
        wrapper.innerHTML = '<p style="grid-column: 1/-1; text-align:center; color:#94a3b8;">Keine Spieler in der daten.csv gefunden.</p>';
        return;
    }
            ...
            <div class="inner-data-box">
                ...
                <div class="stats-grid">
                    ...
                    <div>
                        ...
                        <div class="label" style="margin-top: 0.75rem;">Bilanz (G : V)</div>
                        <div class="value-big">
                            <span class="bilanz-gewonnen">${spieler.siege}</span> : <span class="bilanz-verloren">${spieler.niederlagen}</span>
                        </div>
                        
                        <div class="label" style="margin-top: 0.75rem;">Win-Rate (${winQuote}%)</div>
                    ...
                    <div>
                        ...
                        <div class="label" style="margin-top: 0.5rem;">Legs (Gew : Verl)</div>
                        <div class="value-big">${spieler.legsGew} : ${spieler.legsVerl}</div>
                        
                        <div class="label" style="margin-top: 0.75rem;">Leg-Rate (${legQuote}%)</div>
                        <div class="progress-bg">
