...
function parseSpieltageCSV(text) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        eintraege.push({
            ...
            dartsRaw: row[9] || "", // Nimmt den originalen Text auf (z.B. 25/35/25)
            sg: row[10] || "", 
...
function berechneSpieltagKarten(gewaehlterSpieltag) {
    ...
    spieltagZeilen.forEach(zeile => {
        ...
        if (zeile.spielerKapellenRaw.includes('/')) {
        ...
        spielerAnDiesemTisch.forEach(name => {
            ...
            if (!spielerAbendStats[name]) {
                spielerAbendStats[name] = {
                    ...
                    highfinishes: [],
                    shortGames: [],
                    alleDartsListe: [] // Sammelt alle Darts-Einträge des Abends
                };
            }
            ...
            // Darts-Notation (z.B. "25/35/25") auswerten und sauber formatiert sammeln
            if (zeile.dartsRaw && zeile.dartsRaw !== "" && zeile.dartsRaw !== "0") {
                s.alleDartsListe.push(zeile.dartsRaw);
            }
        });
    });
...
function renderSpieltag(daten, n_spieltag) {
    ...
    daten.forEach(s => {
        ...
        // Zusammenfassung der Darts pro Leg bauen (z.B. "25/35/25 | 18/22/24")
        const dartsAnzeige = s.alleDartsListe.length > 0 ? s.alleDartsListe.join(' | ') : '—';
            ...
            <div class="inner-data-box">
                ...
                <div class="stats-grid">
                    <div>
                        ...
                        <div class="label" style="margin-top: 1rem;">Abend-Bilanz</div>
                        <div class="value-big"><span class="bilanz-gewonnen">${s.siege}</span> : <span class="bilanz-verloren">${s.niederlagen}</span></div>
                        <div class="label" style="margin-top: 1rem;">Win-Rate (${winQuote}%)</div>
                        <div class="progress-bg"><div class="progress-bar green" style="width: ${winQuote}%"></div></div>
                        ${specialsHTML !== '' ? `<div class="label" style="margin-top: 1rem;">Specials</div><div style="margin-top: 0.25rem;">${specialsHTML}</div>` : ''}
                    </div>
                    <div>
                        <div class="label">Abend-Schnitt (AVG)</div>
                        ...
                        <!-- NEU: Hier werden die Darts pro Leg sauber aufgelistet -->
                        <div class="label" style="margin-top: 1.1rem;">Darts pro Leg</div>
                        <div style="font-size: 0.9rem; font-weight: 700; color: #ffffff; margin-top: 0.2rem; word-break: break-all;">
                            ${dartsAnzeige}
                        </div>
                    </div>
                </div>
